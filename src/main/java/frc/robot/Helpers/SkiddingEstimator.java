...
public class SkiddingEstimator {
     ...
     * */
    public static double getSkiddingStandardDeviation(SwerveModuleState[] measuredSwerveStates, SwerveDriveKinematics swerveDriveKinematics) {
        final ChassisSpeeds measuredChassisSpeed = swerveDriveKinematics.toChassisSpeeds(measuredSwerveStates);
        final SwerveModuleState[] idealSwerveStatesGivenNoSkidding = swerveDriveKinematics.toSwerveModuleStates(measuredChassisSpeed);

        double totalSquaredDeviation = 0;
        for (int i = 0; i < 4; i++)
            totalSquaredDeviation += getSquaredDifferenceBetweenTwoSwerveStates(measuredSwerveStates[i], idealSwerveStatesGivenNoSkidding[i]);
     ...
     * */
    private static double getSquaredDifferenceBetweenTwoSwerveStates(SwerveModuleState swerveModuleState1, SwerveModuleState swerveModuleState2) {
        final Translation2d swerveState1VelocityVector = new Translation2d(swerveModuleState1.speedMetersPerSecond, swerveModuleState1.angle),
                swerveState2VelocityVector = new Translation2d(swerveModuleState2.speedMetersPerSecond, swerveModuleState2.angle);
